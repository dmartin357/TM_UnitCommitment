...
dg_index = [(g, s, t) for g in thermal_gens for s in gen_startup_categories[g] for t in time_periods]

m.dg = Var(dg_index, within=Binary) ##
...
m.startup_allowed = Constraint(dg_index)
for g, gen in thermal_gens.items():
    for s,_ in enumerate(gen['startup'][:-1]): ## all but last
        for t in time_periods:
...
SOLUTION_DIR = "C:\\Users\\warha\\OneDrive\\Documents\\Dev\\TM_UnitCommitment\\benchmarks"
os.makedirs(SOLUTION_DIR, exist_ok=True)
log_file = os.path.join(SOLUTION_DIR, "cbc_run.log")
sol_file = os.path.join(SOLUTION_DIR, "cbc_solution.json")

cbc = SolverFactory('cbc')

print("solving")
# NOTE: do NOT pass 'solution' via cbc.options — CBC processes it before -import
# (no model loaded yet) and errors with "Current model not valid".
# Use logfile= to capture full CBC output, then extract values from Pyomo below.
results = cbc.solve(m, options={'ratioGap': 0.01}, tee=True, logfile=log_file)
print(f"CBC log saved → {log_file}")
...
sol = {
    ...
    "ug": {f"{g},{t}": _v(m.ug[g, t]) for g in thermal_gens for t in time_periods},
    "pg": {f"{g},{t}": _v(m.pg[g, t]) for g in thermal_gens for t in time_periods},
    "rg": {f"{g},{t}": _v(m.rg[g, t]) for g in thermal_gens for t in time_periods},
    "vg": {f"{g},{t}": _v(m.vg[g, t]) for g in thermal_gens for t in time_periods},
    "wg": {f"{g},{t}": _v(m.wg[g, t]) for g in thermal_gens for t in time_periods},
    "pw": {f"{w},{t}": _v(m.pw[w, t]) for w in renewable_gens for t in time_periods},
}

with open(sol_file, "w") as fh:
    json.dump(sol, fh, indent=2)
print(f"Solution JSON saved → {sol_file}")
